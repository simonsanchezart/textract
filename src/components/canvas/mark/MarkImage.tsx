...
function MarkImageComponent({ imageData }: { imageData: MarkImageType }) {
  ...
  // simonsanchezart's PR #20 review: dividing unconditionally also grows
  // handles as the user zooms OUT (canvasZoom < 1 means dividing by a
  // fraction, i.e. multiplying), so a zoomed-out view could end up with
  // handles dominating the screen. Below 1x they now just shrink normally
  // along with everything else instead of being compensated.
